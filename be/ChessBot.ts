...
export class ChessBot {
  ...
  public executeMove(from: number, to: number) {
    const move = this.validMoves().filter(
  ...
  public bestMove(): Move {
    ...
    const depth = 2;
    const moves = this.validMoves().sort(() => 0.5 - Math.random());
  ...
  public validMoves(): Move[] {
    return this.trimMoves(this.allMoves()).filter(
      (move) => move.at.toLowerCase() !== "k"
    );
  }
  ...
  private search(depth: number, alpha: number, beta: number): number {
    ...
    if (moves.length === 0) {
      ...
      this.changeTurn();
      if (this.canCaptureKing()) return Number.NEGATIVE_INFINITY;
      this.changeTurn();
      return 0;
    }
  ...
  private canCaptureKing(): boolean {
    ...
    this.pieces.forEach((p, i) => {
      if (p.toLowerCase() === "k" && colorOf(p) !== this.turn) kingPos = i;
    });
  ...
  private trimMoves(moves: Move[]): Move[] {
    ...
    for (const move of moves) {
      this.makeMove(move);
      if (!this.canCaptureKing()) newMoves.push(move);
      this.unmakeMove(move);
    }
  ...
  private allMoves(): Move[] {
    ...
    this.pieces.forEach((p, i) => {
      ...
      switch (c) {
        ...
        case "k":
          moves = moves.concat(
            kingMoves(this.pieces, i, this.whiteCastle, this.blackCastle)
          );
          break;
        default:
          break;
